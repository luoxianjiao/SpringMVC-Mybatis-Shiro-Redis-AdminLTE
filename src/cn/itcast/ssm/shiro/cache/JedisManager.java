package cn.itcast.ssm.shiro.cache;
...
import cn.itcast.ssm.utils.SerializeUtil;
...
public class JedisManager {
     ...
     * @param pre_str
     */
    public void batchDelByPrefix(int dbIndex, String pre_str) {
    	Jedis jedis = null;
        boolean isBroken = false;
        try {
            jedis = getJedis();
            ...
            Set<byte[]> aa = jedis.keys(SerializeUtil.serialize(pre_str +"*"));
//            Long result = jedis.del(key);
//            LoggerUtils.fmtDebug(getClass(), "删除Session结果：%s" , result);
        } catch (Exception e) {
            isBroken = true;
